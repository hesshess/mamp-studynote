...
<body> 
  <h1><a href="/PHP_MYSQL_forum/index.php">게시판</a></h1>
  <p><a href="/PHP_MYSQL_forum/category.php">카테고리 목록보기 & 추가</a>&nbsp;
  <a href="/PHP_MYSQL_forum/create.php">글 작성</a></p>
<?
...
?>
  ...
  <table border=1>
    <tr>
      ...
      <td>작성자</td>
      <?
      $sql = "SELECT p.id, p.title, p.content, p.created, p.writer, c.name FROM post p, category c WHERE p.category_id = c.id ORDER BY p.id DESC LIMIT ".$start.",".$list_num." ";
      $result=mysqli_query($conn, $sql);
      while($row = mysqli_fetch_array($result)){
        $filtered = array(
        'id'=>htmlspecialchars($row['id']),
        'idx'=>htmlspecialchars($cnt),
        ...
        <td><a href="/PHP_MYSQL_forum/update.php?id=<?=$filtered['id']?>">update</a></td>
